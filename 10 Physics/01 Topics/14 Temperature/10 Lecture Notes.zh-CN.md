...
这张图把三件事放在一起：温度计要先和被测物体达到热平衡，Kelvin 温标以绝对零度为起点，加热曲线把升温和物态变化分开。
...
热力学温度用 Kelvin 表示，单位符号为 $\text{K}$。Kelvin 是 SI 基本单位。
...
Celsius 温标和 Kelvin 温标的温度间隔大小相同。变化 $1\ \text{K}$ 与变化 $1\ ^\circ\text{C}$ 一样大。
...
注意不要说 degree kelvin。单位名称就是 kelvin。

## 5. Celsius 数值与温度变化量

要区分“温度数值”和“温度变化量”。

在气体定律或热力学公式中，若使用绝对温度数值，要用 Kelvin。
...
- [[10 Physics/01 Topics/15 Ideal Gases/00 Overview|Ideal Gases]] 会在气体定律和分子动理论中使用 Kelvin 温度。
