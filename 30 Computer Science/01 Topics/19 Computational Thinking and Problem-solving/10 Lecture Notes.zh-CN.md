...
- 19.2 Recursion：递归的本质特征、在编程语言中的表达方式、编写与跟踪递归；编译器如何用栈翻译递归并完成解栈。
...
**在编程语言中如何表达。** 递归函数和别的函数写法完全一样；递归只是一个 `IF`：要么返回基线值，要么返回 `f(更小参数)`。语言提供机制——每次调用在调用栈上获得自己的一套局部变量和参数，结果在调用解栈时传回。
...
跟踪 `Factorial(3)` 的调用栈。每次调用压入一个栈帧并等待它发起的调用返回；然后各帧按相反顺序解栈。
    ...
    调用 Factorial(2)
      N = 2，非基线，需要 2 * Factorial(1)          [压入帧 2]
        调用 Factorial(1)
          ...
          1 * 1 = 1，RETURN 1                       [帧 1 解栈]
      2 * 1 = 2，RETURN 2                           [帧 2 解栈]
  3 * 2 = 6，RETURN 6                               [帧 3 解栈]
结果：6
```
...
当某个栈帧终于命中基线情形并返回时，返回值被传回发起调用的那一帧，然后该帧被**弹出**。控制权回到发起调用的栈帧，从它当初停下的地方继续。这种按相反顺序逐帧弹出的过程叫做**解栈（unwinding，回溯）**。所以 `RETURN N * Factorial(N - 1)` 的意思是：调用 `Factorial(N - 1)`，压入一个栈帧并等待；等它返回后，把结果乘以 `N`，再返回*那个值*。栈正是让每次调用都能记住自己停在哪、拿到结果后该干什么的机制。
...
5. 递归题先写基线情形，再写递归情形，然后跟踪调用栈一层层压下去、再把结果一层层解栈传回。
...
13. 解释编译器为每次递归调用往栈里压了什么，“解栈”又是什么意思。
